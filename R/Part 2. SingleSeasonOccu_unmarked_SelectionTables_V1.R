...
files=data.frame(File=list.files(path='data/SingleSeason - from SelectionTables/RBS_Grid2',
                                 recursive=T,pattern='txt$',full.names = T))

FileName <- basename(files$File)

FileNameShort <- str_split_fixed(FileName,pattern = '.BirdNET',n=2)[,1]
FileNameSplit <- str_split_fixed(FileName,pattern = '_',n=3)


## compile file list, effort

  files$Site=FileNameSplit[,1]
  
  files$Date=as.Date( paste(
    substr(FileNameSplit[,2],7,8),
    substr(FileNameSplit[,2],5,6),
    substr(FileNameSplit[,2],1,4), sep='/'),"%m/%d/%y")
  
   
  files$Hour=as.numeric(substr(FileNameSplit[,3],1,2))
...
for(f in 1:length(files$File)){
  ...
  if(dim(tmp)[1] != 0){
    ...
    FileNameShort <- str_split_fixed(files$File[f],pattern = '.BirdNET',n=2)[,1]
    FileNameSplit <- str_split_fixed(files$File[f],pattern = '_',n=3)
    
    
    tmp$Site=FileNameSplit[,1]
    ...
    tmp$Hour=as.numeric(substr(FileNameSplit[,3],1,2))
    tmp$Min=as.numeric(substr(FileNameSplit[,3],3,4))
...
write.csv(data.frame(eh_ASCI,ID=rownames(effort_days)), 'data/SingleSeason - from SelectionTables/output_data/Ashy Cisticola.csv', row.names = F)
write.csv(data.frame(eh_NUWO,ID=rownames(effort_days)),  'data/SingleSeason - from SelectionTables/output_data/Nubian Woodpecker.csv', row.names = F)
write.csv(data.frame(eh_GRWH,ID=rownames(effort_days)), 'data/SingleSeason - from SelectionTables/output_data/Green Woodhoopoe.csv', row.names = F)
write.csv(data.frame(eh_AFPI,ID=rownames(effort_days)), 'data/SingleSeason - from SelectionTables/output_data/African Pipit.csv', row.names = F)

write.csv(data.frame(effort_days,ID=rownames(effort_days)), 'data/SingleSeason - from SelectionTables/output_data/effort_days.csv', row.names = F)
write.csv(data.frame(effort_hrs,ID=rownames(effort_hrs)), 'data/SingleSeason - from SelectionTables/output_data/effort_hrs.csv', row.names = F)
